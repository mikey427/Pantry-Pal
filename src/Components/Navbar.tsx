import React, { useState, useEffect } from "react";
import { Link } from "react-router-dom";
import Home from "../Pages/Home";
import { useTheme } from "./ThemeProvider";

type Props = {};

export default function Navbar({}: Props) {
	const { theme, toggleTheme } = useTheme();

	// useEffect(() => {
	// 	// document.querySelector("html")!.setAttribute("data-theme", theme);
	// }, [theme]);
	return (
		// <div className="flex mx-auto w-1/2 items-center top-0 h-12 mt-4">
		// 	<Link
		// 		className=" flex items-center m-auto bg bg-red-500 w-48 h-full rounded-2xl hover:bg-red-700 hover:shadow-lg"
		// 		to="/"
		// 	>
		// 		<p className="mx-auto">Home</p>
		// 	</Link>
		// 	<Link
		// 		className=" flex items-center justify-center bg bg-orange-500 w-48 h-full rounded-2xl hover:bg-orange-700 hover:shadow-lg"
		// 		to="/meal_planner"
		// 	>
		// 		<svg
		// 			className="mr-1"
		// 			xmlns="http://www.w3.org/2000/svg"
		// 			height="24"
		// 			viewBox="0 -960 960 960"
		// 			width="24"
		// 		>
		// 			<path d="M200-80q-33 0-56.5-23.5T120-160v-560q0-33 23.5-56.5T200-800h40v-80h80v80h320v-80h80v80h40q33 0 56.5 23.5T840-720v560q0 33-23.5 56.5T760-80H200Zm0-80h560v-400H200v400Zm0-480h560v-80H200v80Zm0 0v-80 80Zm280 240q-17 0-28.5-11.5T440-440q0-17 11.5-28.5T480-480q17 0 28.5 11.5T520-440q0 17-11.5 28.5T480-400Zm-160 0q-17 0-28.5-11.5T280-440q0-17 11.5-28.5T320-480q17 0 28.5 11.5T360-440q0 17-11.5 28.5T320-400Zm320 0q-17 0-28.5-11.5T600-440q0-17 11.5-28.5T640-480q17 0 28.5 11.5T680-440q0 17-11.5 28.5T640-400ZM480-240q-17 0-28.5-11.5T440-280q0-17 11.5-28.5T480-320q17 0 28.5 11.5T520-280q0 17-11.5 28.5T480-240Zm-160 0q-17 0-28.5-11.5T280-280q0-17 11.5-28.5T320-320q17 0 28.5 11.5T360-280q0 17-11.5 28.5T320-240Zm320 0q-17 0-28.5-11.5T600-280q0-17 11.5-28.5T640-320q17 0 28.5 11.5T680-280q0 17-11.5 28.5T640-240Z" />
		// 		</svg>
		// 		<p className="">Meal Planner</p>
		// 	</Link>
		// 	<Link
		// 		className=" flex items-center justify-center bg bg-yellow-300 w-48 h-full rounded-2xl hover:bg-yellow-500 hover:shadow-lg"
		// 		to="/ingredients"
		// 	>
		// 		<svg
		// 			className="mr-1"
		// 			xmlns="http://www.w3.org/2000/svg"
		// 			height="24"
		// 			viewBox="0 -960 960 960"
		// 			width="24"
		// 		>
		// 			<path d="M640-80q-100 0-170-70t-70-170q0-100 70-170t170-70q100 0 170 70t70 170q0 100-70 170T640-80Zm0-80q66 0 113-47t47-113q0-66-47-113t-113-47q-66 0-113 47t-47 113q0 66 47 113t113 47Zm-480 0q-33 0-56.5-23.5T80-240v-304q0-8 1.5-16t4.5-16l80-184h-6q-17 0-28.5-11.5T120-800v-40q0-17 11.5-28.5T160-880h280q17 0 28.5 11.5T480-840v40q0 17-11.5 28.5T440-760h-6l66 152q-19 10-36 21t-32 25l-84-198h-96l-92 216v304h170q5 21 13.5 41.5T364-160H160Zm480-440q-42 0-71-29t-29-71q0-42 29-71t71-29v200q0-42 29-71t71-29q42 0 71 29t29 71H640Z" />
		// 		</svg>
		// 		<p className="">Ingredients</p>
		// 	</Link>
		// 	<Link
		// 		className=" flex items-center m-auto bg bg-green-500 w-48 h-full rounded-2xl hover:bg-green-700 hover:shadow-lg"
		// 		to="/shopping_list"
		// 	>
		// 		<p className="mx-auto">Shopping List</p>
		// 	</Link>
		// 	<Link
		// 		className=" flex items-center m-auto bg bg-blue-500 w-48 h-full rounded-2xl hover:bg-blue-700 hover:shadow-lg"
		// 		to="/saved_meals"
		// 	>
		// 		<p className="mx-auto">Saved Meals</p>
		// 	</Link>
		// 	{/* <Link className=' flex items-center m-auto bg bg-purple-500 w-48 h-full rounded-2xl hover:bg-purple-700 hover:shadow-lg' to="/" >
		//         <p className='mx-auto'>Settings</p>
		//     </Link>
		//     <Link className=' flex items-center m-auto bg bg-pink-500 w-48 h-full rounded-2xl hover:bg-pink-700 hover:shadow-lg' to="/" >
		//         <p className='mx-auto'>About</p>
		//     </Link> */}
		// </div>
		<div className="navbar">
			<div className="navbar-start">
				<div className="dropdown">
					<div tabIndex={0} role="button" className="btn btn-ghost lg:hidden">
						<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" />
						</svg>
					</div>
					<ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow rounded-box w-52">
						<li className="">
							<Link className="" to="/">
								<p className="mx-auto">Home</p>
							</Link>
						</li>
						<li>
							<Link className="" to="/meal_planner">
								<svg className="mr-1" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
									<path d="M200-80q-33 0-56.5-23.5T120-160v-560q0-33 23.5-56.5T200-800h40v-80h80v80h320v-80h80v80h40q33 0 56.5 23.5T840-720v560q0 33-23.5 56.5T760-80H200Zm0-80h560v-400H200v400Zm0-480h560v-80H200v80Zm0 0v-80 80Zm280 240q-17 0-28.5-11.5T440-440q0-17 11.5-28.5T480-480q17 0 28.5 11.5T520-440q0 17-11.5 28.5T480-400Zm-160 0q-17 0-28.5-11.5T280-440q0-17 11.5-28.5T320-480q17 0 28.5 11.5T360-440q0 17-11.5 28.5T320-400Zm320 0q-17 0-28.5-11.5T600-440q0-17 11.5-28.5T640-480q17 0 28.5 11.5T680-440q0 17-11.5 28.5T640-400ZM480-240q-17 0-28.5-11.5T440-280q0-17 11.5-28.5T480-320q17 0 28.5 11.5T520-280q0 17-11.5 28.5T480-240Zm-160 0q-17 0-28.5-11.5T280-280q0-17 11.5-28.5T320-320q17 0 28.5 11.5T360-280q0 17-11.5 28.5T320-240Zm320 0q-17 0-28.5-11.5T600-280q0-17 11.5-28.5T640-320q17 0 28.5 11.5T680-280q0 17-11.5 28.5T640-240Z" />
								</svg>
								<p className="">Meal Planner</p>
							</Link>
						</li>
						<li>
							<Link className="" to="/ingredients">
								<svg className="mr-1" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
									<path d="M640-80q-100 0-170-70t-70-170q0-100 70-170t170-70q100 0 170 70t70 170q0 100-70 170T640-80Zm0-80q66 0 113-47t47-113q0-66-47-113t-113-47q-66 0-113 47t-47 113q0 66 47 113t113 47Zm-480 0q-33 0-56.5-23.5T80-240v-304q0-8 1.5-16t4.5-16l80-184h-6q-17 0-28.5-11.5T120-800v-40q0-17 11.5-28.5T160-880h280q17 0 28.5 11.5T480-840v40q0 17-11.5 28.5T440-760h-6l66 152q-19 10-36 21t-32 25l-84-198h-96l-92 216v304h170q5 21 13.5 41.5T364-160H160Zm480-440q-42 0-71-29t-29-71q0-42 29-71t71-29v200q0-42 29-71t71-29q42 0 71 29t29 71H640Z" />
								</svg>
								<p className="">Ingredients</p>
							</Link>
						</li>
						<li>
							<Link className="" to="/shopping_list">
								<p className="mx-auto">Shopping List</p>
							</Link>
						</li>
						<li>
							<Link className="" to="/saved_meals">
								<p className="mx-auto">Saved Meals</p>
							</Link>
						</li>
					</ul>
				</div>
				<a className="btn btn-ghost text-xl">Pantry Pal</a>
			</div>
			<div className="navbar-center hidden lg:flex">
				<ul className="menu menu-horizontal px-1">
					<li>
						<Link className="" to="/">
							<p className="mx-auto">Home</p>
						</Link>
					</li>
					<li>
						<Link className="" to="/meal_planner">
							<svg className="mr-1" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
								<path d="M200-80q-33 0-56.5-23.5T120-160v-560q0-33 23.5-56.5T200-800h40v-80h80v80h320v-80h80v80h40q33 0 56.5 23.5T840-720v560q0 33-23.5 56.5T760-80H200Zm0-80h560v-400H200v400Zm0-480h560v-80H200v80Zm0 0v-80 80Zm280 240q-17 0-28.5-11.5T440-440q0-17 11.5-28.5T480-480q17 0 28.5 11.5T520-440q0 17-11.5 28.5T480-400Zm-160 0q-17 0-28.5-11.5T280-440q0-17 11.5-28.5T320-480q17 0 28.5 11.5T360-440q0 17-11.5 28.5T320-400Zm320 0q-17 0-28.5-11.5T600-440q0-17 11.5-28.5T640-480q17 0 28.5 11.5T680-440q0 17-11.5 28.5T640-400ZM480-240q-17 0-28.5-11.5T440-280q0-17 11.5-28.5T480-320q17 0 28.5 11.5T520-280q0 17-11.5 28.5T480-240Zm-160 0q-17 0-28.5-11.5T280-280q0-17 11.5-28.5T320-320q17 0 28.5 11.5T360-280q0 17-11.5 28.5T320-240Zm320 0q-17 0-28.5-11.5T600-280q0-17 11.5-28.5T640-320q17 0 28.5 11.5T680-280q0 17-11.5 28.5T640-240Z" />
							</svg>
							<p className="">Meal Planner</p>
						</Link>
					</li>
					<li>
						<Link className="" to="/ingredients">
							<svg className="mr-1" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
								<path d="M640-80q-100 0-170-70t-70-170q0-100 70-170t170-70q100 0 170 70t70 170q0 100-70 170T640-80Zm0-80q66 0 113-47t47-113q0-66-47-113t-113-47q-66 0-113 47t-47 113q0 66 47 113t113 47Zm-480 0q-33 0-56.5-23.5T80-240v-304q0-8 1.5-16t4.5-16l80-184h-6q-17 0-28.5-11.5T120-800v-40q0-17 11.5-28.5T160-880h280q17 0 28.5 11.5T480-840v40q0 17-11.5 28.5T440-760h-6l66 152q-19 10-36 21t-32 25l-84-198h-96l-92 216v304h170q5 21 13.5 41.5T364-160H160Zm480-440q-42 0-71-29t-29-71q0-42 29-71t71-29v200q0-42 29-71t71-29q42 0 71 29t29 71H640Z" />
							</svg>
							<p className="">Ingredients</p>
						</Link>
					</li>
					<li>
						<Link className="" to="/shopping_list">
							<p className="mx-auto">Shopping List</p>
						</Link>
					</li>
					<li>
						<Link className="" to="/saved_meals">
							<p className="mx-auto">Saved Meals</p>
						</Link>
					</li>
				</ul>
			</div>
			{/* <div
				className="navbar-end"
				onClick={event => {
					setTheme(theme === "light" ? "dark" : "light");
				}}
			>
				<a className="btn">Switch Theme</a>
			</div> */}
			<div className="flex justify-end navbar-end">
				<label className="w-10 swap swap-rotate">
					{/* this hidden checkbox controls the state */}
					<input
						type="checkbox"
						className="hidden"
						onClick={event => {
							// setTheme(theme === "light" ? "dark" : "light");
							toggleTheme();
						}}
					/>
					{/* sun icon */}
					<svg className="swap-off fill-current w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
						<path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
					</svg>

					{/* moon icon */}
					<svg className="swap-on fill-current w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
						<path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
					</svg>
				</label>
			</div>
		</div>
	);
}
